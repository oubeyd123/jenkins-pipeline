...
pipeline {
    ...
    environment {
        REGISTRY           = 'docker.io/YOUR_DOCKERHUB_ORG'   // TODO: set your registry namespace
        REGISTRY_CRED_ID   = 'dockerhub-token'                // Jenkins credential ID
        BUILD_AGENT_LABEL  = 'linux-build'                    // any Linux agent with JDK/Maven/Docker
        DEPLOY_AGENT_LABEL = 'wso2-target-server'              // the agent that IS the target server
    }
    ...
    post {
        always {
            echo "Finished: branch=${env.BRANCH_NAME ?: 'n/a'} release=${env.IS_RELEASE ?: 'n/a'}"
        }
    }
}
